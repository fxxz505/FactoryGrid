...
export function entityConnectionDirections(project: FactoryProject, entity: FactoryEntity): Direction[] {
  ...
  return normalizeConnections(machinePorts(entity))
}
...
export function connectedDirections(project: FactoryProject, entity: FactoryEntity): Direction[] {
  ...
  directions().forEach((direction) => {
    ...
    if (neighbor.type !== 'belt' && machinePorts(neighbor).includes(opposite(direction))) {
      dirs.add(direction)
      return
    }
...
function machinePorts(entity: FactoryEntity): Direction[] {
  if (entity.kind === 'source') return [entity.direction]
  if (entity.type === 'splitter') return splitterPorts(entity.direction)
  if (entity.type === 'merger') return directions()
  if (entity.type === 'hub' || entity.type === 'trash') return [opposite(entity.direction)]
  return [entity.direction, opposite(entity.direction)]
}
